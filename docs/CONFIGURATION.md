...
- `auto` tries the portal first and falls back to evdev when the portal or the
  host-application registry cannot be reached when the daemon starts.
...
decision. It switches to evdev only when the portal or the host-application
registry cannot be reached when the daemon starts. If the portal disappears
later, `auto` behaves like `portal`: the daemon logs each failed attempt and
keeps retrying, so a portal that restarts does not move the user onto raw input
access.
