...
const VOXTYPE_HYPRLAND_CONFIG: &str = r#"pre_output_command = "hyprctl dispatch submap voxtype_suppress"
post_output_command = "hyprctl dispatch submap reset""#;

/// Voxtype config snippet for Sway
const VOXTYPE_SWAY_CONFIG: &str = r#"pre_output_command = "swaymsg mode voxtype_suppress"
...
# Block modifier keys in the suppression mode
...
const VOXTYPE_RIVER_CONFIG: &str = r#"pre_output_command = "riverctl enter-mode voxtype_suppress"
...
fn hyprland_install() -> anyhow::Result<()> {
    ...
    if voxtype_config.exists() {
        ...
        if content.contains("pre_output_command") {
            print_success("Voxtype config already has output hooks");
        } else {
            ...
            print_success("Added output hooks to voxtype config");
        }
    } else {
...
fn hyprland_uninstall() -> anyhow::Result<()> {
    ...
    print_warning("Remember to remove output hooks from voxtype config if no longer needed");
    println!("\n---");
...
fn hyprland_status() {
    ...
    if voxtype_config.exists() {
        if let Ok(content) = std::fs::read_to_string(&voxtype_config) {
            if content.contains("pre_output_command") && content.contains("hyprctl") {
                print_success("Voxtype hooks: configured for Hyprland");
...
fn sway_status() {
    ...
    if voxtype_config.exists() {
        if let Ok(content) = std::fs::read_to_string(&voxtype_config) {
            if content.contains("pre_output_command") && content.contains("swaymsg") {
                print_success("Voxtype hooks: configured for Sway");
            } else if content.contains("pre_output_command") {
...
fn river_status() {
    ...
    if voxtype_config.exists() {
        if let Ok(content) = std::fs::read_to_string(&voxtype_config) {
            if content.contains("pre_output_command") && content.contains("riverctl") {
                print_success("Voxtype hooks: configured for River");
            } else if content.contains("pre_output_command") {
...
"#;
    ...
    #[test]
    fn test_hyprland_config_contains_submap() {
    ...
    #[test]
    fn test_sway_config_contains_mode() {
    ...
    #[test]
    fn test_river_config_contains_mode() {
