...
fn language_token_id(lang: &str) -> i64 {
    ...
    let offset = match lang {
        ...
        "su" => 98,
        _ => {
            tracing::warn!("Unknown language code '{}', defaulting to English", lang);
            0 // English
        }
    };
...
impl OpenVinoTranscriber {
    ...
    fn compile_models(
        ...
        config: &OpenVinoConfig,
    ) -> Result<CompiledModels, TranscribeError> {
        ...
        let mut compiled_encoder = core
            .compile_model(&encoder_model, Self::parse_device(&config.device))
            .map_err(|e| {
                if is_npu {
                    TranscribeError::InitFailed(format!(
        ...
        let mut compiled_decoder = core
            .compile_model(&decoder_model, Self::parse_device(&config.device))
            .map_err(|e| {
                TranscribeError::InitFailed(format!(
    ...
    fn ensure_compiled(
        &self,
    ) -> Result<std::sync::MutexGuard<'_, Option<CompiledModels>>, TranscribeError> {
    ...
    fn run_inference(&self, samples: &[f32]) -> Result<Vec<u32>, TranscribeError> {
        ...
        let mel_shape =
            Shape::new(&[1, WHISPER_N_MELS as i64, WHISPER_N_FRAMES as i64]).map_err(|e| {
        ...
        for step in 0..max_tokens {
            ...
            let ids_shape = Shape::new(&[1, input_tokens.len() as i64]).map_err(|e| {
                TranscribeError::InferenceFailed(format!("Failed to create input_ids shape: {}", e))
            })?;
            ...
            ids_tensor
                .get_data_mut::<i64>()
                .map_err(|e| {
                    TranscribeError::InferenceFailed(format!("Failed to get input_ids data: {}", e))
                })?
            ...
            let logits_tensor = models
                .decoder_request
                .get_output_tensor_by_index(0)
                .map_err(|e| {
                    TranscribeError::InferenceFailed(format!("Failed to get decoder output: {}", e))
                })?;
...
impl Transcriber for OpenVinoTranscriber {
    ...
    fn prepare(&self) {
        ...
        tracing::info!(
            "Compiling OpenVINO models for {} (triggered by prepare())...",
            self.config.device
        );
