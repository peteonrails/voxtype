//! Dolphin speech-to-text transcription (dictation-optimized CTC encoder)
//!
//! Uses Dolphin model via ONNX Runtime. Dolphin is a CTC encoder optimized
//! for dictation with different Fbank parameters from SenseVoice/Paraformer:
//! Hann window, 31.25ms frame length, no LFR stacking, no pre-emphasis.
//!
//! Pipeline: Audio (f32, 16kHz) -> Fbank (80-dim, Hann, 31.25ms) -> CMVN -> ONNX -> CTC decode -> text
...
impl DolphinTranscriber {
    pub fn new(config: &DolphinConfig) -> Result<Self, TranscribeError> {
        ...
        // Dolphin uses Hann window, 31.25ms frame, no pre-emphasis, no LFR
        let fbank_extractor = FbankExtractor::new(FbankConfig::dolphin_default());
...
impl Transcriber for DolphinTranscriber {
    fn transcribe(&self, samples: &[f32]) -> Result<String, TranscribeError> {
        ...
        tracing::debug!("Transcribing {:.2}s of audio with Dolphin", duration_secs);

        let start = std::time::Instant::now();

        // 1. Fbank extraction (Hann window, 31.25ms, no pre-emphasis)
        ...
        // 2. CMVN normalization (no LFR for Dolphin)
