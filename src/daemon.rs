...
fn build_deepgram_config(
    ...
    crate::transcribe::deepgram::DeepgramConfig {
        ...
        finish_timeout_secs: whisper_config.streaming_finish_timeout_secs.unwrap_or(15),
    }
}
...
impl Daemon {
    ...
    async fn finish_streaming_recording(
        ...
        let timeout_secs = self.config.whisper.streaming_finish_timeout_secs.unwrap_or(15);
        match tokio::time::timeout(Duration::from_secs(timeout_secs), (*stream).finish()).await {
            Ok(result) => result,
            Err(_) => {
                tracing::warn!("Deepgram stream finish timed out after {timeout_secs}s");
