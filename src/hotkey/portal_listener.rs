...
/// How long a stop waits for the desktop to acknowledge closing the session
/// before the daemon shuts down without it.
...
async fn run_session(
    ...
    close_session(&session_proxy, CLOSE_SESSION_TIMEOUT).await;
...
async fn close_session(session: &Proxy<'_>, timeout: Duration) {
    match tokio::time::timeout(timeout, session.call::<_, _, ()>("Close", &())).await {
        Ok(Ok(())) => {}
        Ok(Err(error)) => {
            tracing::debug!("Could not close XDG GlobalShortcuts session: {}", error);
        }
        Err(_) => tracing::warn!(
            "The desktop did not answer closing the XDG GlobalShortcuts session within {:?}",
            timeout
        ),
    }
}
...
/// Registers the host application and reads the portal interface versions, as
/// the daemon does on its first connection. Registering is part of the check:
/// the portal resolves the application ID through its own desktop file lookup,
/// so a failure here is the one the daemon would hit.
pub(crate) async fn probe_portal() -> Result<(u32, u32), HotkeyError> {
    let connection = Connection::session()
        .await
        .map_err(HotkeyError::PortalUnavailable)?;
    register_host(&connection).await?;
...
mod tests {
    ...
    #[tokio::test]
    async fn closing_a_session_gives_up_when_the_desktop_does_not_answer() {
        let (server_stream, client_stream) =
            tokio::net::UnixStream::pair().expect("Unix stream pair should be created");
        let guid = zbus::Guid::generate();
        let server_builder = zbus::connection::Builder::unix_stream(server_stream)
            ...
            .expect("server GUID should be accepted")
            .p2p()
            .serve_at("/hanging/session", HangingSession)
            .expect("session interface should be served");
        let client_builder = zbus::connection::Builder::unix_stream(client_stream).p2p();
        let (server, client) = tokio::try_join!(server_builder.build(), client_builder.build())
            .expect("peer connections should be established");
        ...
        .await
        .expect("session proxy should be created");

        let closed = tokio::time::timeout(
            Duration::from_secs(1),
            close_session(&session, Duration::from_millis(50)),
        )
        .await;

        assert!(
            closed.is_ok(),
            "close_session should return once its own timeout passes"
        );
        drop(server);
    }
