...
fn guidance_backend(state: &HotkeyState) -> Vec<Line<'_>> {
    ...
    match &state.backend {
        ...
        Backend::Known(HotkeyBackend::Auto) => {
            lines.push(Line::from(
                "Auto tries the portal first. It uses evdev only when the portal or host-application registry cannot be reached when the daemon starts. A portal that disappears later is retried.",
            ));
            lines.push(Line::from(""));
            lines.push(Line::from(
